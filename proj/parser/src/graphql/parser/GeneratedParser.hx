...
/* GENERATED BY gen_parser.rb -- DO NOT EDIT!!! */
/* */
/* based on: https://raw.githubusercontent.com/graphql/graphql-js/v0.13.2/src/language/parser.js */
/* */
...
import BR_L syntaxError } from '../error';
import type BR_L GraphQLError } from '../error';
import BR_L createLexer, TokenKind, getTokenDesc } from './lexer';
import type BR_L Lexer, TokenKindEnum } from './lexer';
import type BR_L
  Location,
  Token,
  NameNode,
...
public function parseDocument(lexer: Lexer): DocumentNode {
  ...
  return {
    kind: Kind.DOCUMENT,
    definitions:definitions, /* ESIOK: stupid inferred key name */
    loc: loc(lexer, start),
  };
}
...
private function parseDefinition(lexer: Lexer): DefinitionNode {
  if (peek(lexer, TokenKind.NAME)) {
    switch (lexer.token.value) {
      ...
      case 'schema' | /*CFT*/ 'scalar' | /*CFT*/ 'type' | /*CFT*/ 'interface' | /*CFT*/ 'union' | /*CFT*/ 'enum' | /*CFT*/ 'input' | /*CFT*/ 'extend' | /*CFT*/ 'directive':
...
private function parseVariableDefinition(lexer: Lexer): VariableDefinitionNode {
  ...
  return {
    ...
    type: { expect(lexer, TokenKind.COLON) ;  parseTypeReference(lexer) ; } /* WTH (,) block */,
...
private function parseArgument(lexer: Lexer): ArgumentNode {
  ...
  return {
    kind: Kind.ARGUMENT,
    name: parseName(lexer),
    value: { expect(lexer, TokenKind.COLON) ;  parseValueLiteral(lexer, false) ; } /* WTH (,) block */,
    loc: loc(lexer, start),
  };
}
...
private function parseConstArgument(lexer: Lexer): ArgumentNode {
  ...
  return {
    kind: Kind.ARGUMENT,
    name: parseName(lexer),
    value: { expect(lexer, TokenKind.COLON) ;  parseConstValue(lexer) ; } /* WTH (,) block */,
    loc: loc(lexer, start),
  };
}
  ...
  lexer: Lexer): SomeFragmentNode 
{
  ...
  return /* CSMT */ cast {
    ...
    typeCondition:typeCondition, /* ESIOK: stupid inferred key name */
    directives: parseDirectives(lexer, false),
    selectionSet: parseSelectionSet(lexer),
    loc: loc(lexer, start),
  };
...
private function parseObjectField(lexer: Lexer, isConst:Bool): ObjectFieldNode {
  ...
  return {
    ...
    name: parseName(lexer),
    value: { expect(lexer, TokenKind.COLON) ;  parseValueLiteral(lexer, isConst) ; } /* WTH (,) block */,
    loc: loc(lexer, start),
  };
}
 ...
 *   - InputObjectTypeDefinition
 */
private function parseTypeExtension(lexer: Lexer): TypeExtensionNode {
 ...
 */
private function skip(lexer: Lexer, kind: TokenKindEnum):Bool {
 ...
 */
private function expect(lexer: Lexer, kind: TokenKindEnum): Token {
  var token = lexer.token;
  if (token.kind == kind) {
    lexer.advance();
    return token;
